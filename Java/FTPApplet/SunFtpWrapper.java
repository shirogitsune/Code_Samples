...
public class SunFtpWrapper extends FtpClient {
	...
	public boolean uploadFile (String localFile, String serverFile) throws IOException {
		int i = 0;
		percentage = 0; //<- Added this for progress monitoring
		byte[] bytesIn = new byte[1024];
		FileInputStream in = new FileInputStream(localFile);
		BufferedOutputStream out = new BufferedOutputStream(put(serverFile));
		while ((i = in.read(bytesIn)) >= 0) {
			/* JP: Hacked to add progress monitoring of the file transfer so we don't get bored...*/
			label1.setText("Uploaded: "+((in.getChannel().position()/1024))+"KB");
			label2.setText("Remaining: "+((in.getChannel().size()-in.getChannel().position())/1024)+"KB");
			label3.setText(percentage+"% complete");
			percentage = (int)((100*in.getChannel().position())/in.getChannel().size());
			/*/End Hack */
			out.write(bytesIn, 0, i);
		}
		in.close();
	...
	/* JP: Declarations for the hack above... */
